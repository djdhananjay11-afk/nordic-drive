...
export async function PremiumCarDetailPage({ car }: CarDetailPageProps) {
  const locale = await getRequestLocale();
  const experience = getCarDetailExperience(car);
  const similarVehicles = nordicCars
    .filter(
      (candidate) => candidate.segment === car.segment && candidate.modelSlug !== car.modelSlug,
    )
    .slice(0, 3);
  ...
  const specs = [
    {
      ...
      detail: "pending source verification",
    },
    {
      icon: Snowflake,
      label: "Winter range",
      value: `${car.winterRangeKm} km`,
      detail: "Nordic estimate",
    },
    {
      icon: Zap,
      label: "10-80% charging",
      value: `${car.chargingMinutes} min`,
      detail: `${car.fastChargingKw} kW peak`,
    },
    {
      icon: Gauge,
      label: "0-100 km/h",
      value: `${car.accelerationSeconds.toFixed(1)} s`,
      detail: car.drivetrain,
    },
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#eef4f8_42%,#ffffff_100%)] text-slate-950">
      <section className="relative overflow-hidden px-5 pb-12 pt-24">
        ...
        <div className="relative mx-auto max-w-7xl">
          <Button
            asChild
            className="mb-8 bg-white/72 text-slate-900 hover:bg-white"
            size="sm"
            variant="glass"
          >
            <Link href={localizePath(locale, "/cars") as Route}>
              <ArrowLeft className="mr-2 size-4" />
              {locale === "no" ? "Alle biler" : "All cars"}
            </Link>
          </Button>
                ...
                <Button asChild className="bg-slate-950 text-white hover:bg-slate-800" size="xl">
                  <Link
                    href={
                      localizePath(
                        locale,
                        `/compare?vehicles=${encodeURIComponent(`${car.brandSlug}:${car.modelSlug}`)}`,
                      ) as Route
                    }
                  >
                    {locale === "no" ? "Sammenlign" : "Compare now"}
                  </Link>
                </Button>
                <Button
                  asChild
                  className="bg-white/72 text-slate-950 hover:bg-white"
                  size="xl"
                  variant="glass"
                >
                  <Link href={localizePath(locale, "/ai") as Route}>
                    <BrainCircuit className="mr-2 size-4" />
                    {locale === "no" ? "Spør Nordic AI" : "Ask Nordic AI"}
                  </Link>
                </Button>
              </div>
            </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[0.82fr_1.18fr]">
          <GlassCard className="bg-slate-950 p-7 text-white shadow-sm">
            <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.22em] text-white/48">
              ...
              {locale === "no" ? "Foreløpig omtale" : "Provisional overview"}
            </div>
            <p className="mt-6 text-2xl font-semibold leading-9">{experience.aiSummary}</p>
            <div className="mt-7 grid gap-3 sm:grid-cols-3">
              {car.highlights.map((highlight) => (
                <div
                  className="rounded-md border border-white/10 bg-white/10 p-4 text-sm font-semibold"
                  key={highlight}
                >
                  {highlight}
                </div>
            ...
            <div className="mt-4 grid gap-3">
              {experience.variants.map((variant) => (
                ...
                >
                  ...
                  <VariantMetric
                    label="0-100"
                    value={`${variant.accelerationSeconds.toFixed(1)} s`}
                  />
                </GlassCard>
              ))}
            </div>
      ...
      <section className="px-5 py-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-5">
            <VehicleImage car={car} className="aspect-video w-full" />
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-5 text-sm leading-6 text-slate-600">
              {locale === "no"
                ? "Ingen verifiserte videoer er tilgjengelige for denne modellen ennå."
                : "No verified videos are available for this model yet."}
            </p>
          </div>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {similarVehicles.map((vehicle) => (
              <Link
                ...
                href={
                  localizePath(locale, `/cars/${vehicle.brandSlug}/${vehicle.modelSlug}`) as Route
                }
      ...
      <section className="px-5 pb-24 pt-10">
        <div className="mx-auto max-w-7xl">
          <GlassCard className="relative overflow-hidden bg-slate-950 p-8 text-white shadow-sm md:p-12">
            <div
              className={cn(
                "absolute inset-y-0 right-0 w-1/2 bg-gradient-to-l opacity-30",
                car.colorClass,
              )}
            />
            <div className="relative max-w-2xl">
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-white/48">
                Comparison studio
              </p>
              <h2 className="mt-4 text-3xl font-semibold md:text-5xl">
                Compare {car.model} against Norway's best EVs.
              </h2>
              <p className="mt-4 text-base leading-7 text-white/62">
                Benchmark price, winter range, charging speed, towing, and ownership practicality in
                one premium view.
              </p>
              <Button asChild className="mt-7 bg-white text-slate-950 hover:bg-white/88" size="xl">
                ...
                >
                  {locale === "no" ? "Åpne sammenligning" : "Open comparison"}
                </Link>
              </Button>
            </div>
          </GlassCard>
